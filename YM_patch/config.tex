...
\def\titleZh   {陽明大學LaTeX論文樣板(中文)}   % Chinese title
\def\titleEn   {YM LaTeX Thesis Template(Chinese)} % English title
...
\def\degreedateROC{中華民國~\zhdigits{107}~年~六~月} % YM use O not 零
...
\def\wmContent{logo-YM.jpg} % 圖檔名或文字 | image filename or text.(中央不用加，若你想加，樣板已附上，填入 ``logo-YM.jpg'' 即可)
