...
\def\deptshort {物理}
\def\deptZh    {物理研究所}%XX學系XXX碩士班(請參考「中央大學學位論文撰寫體例參考」附錄)
\def\deptEn    {Department of Physics}
\def\degreeZh  {碩士} % 碩士/博士
\def\degreeEn  {Master} % Master/PHD? | Master thesis/Doctoral dissertation
\def\titleZh   {暨南大學LaTeX論文樣板(中文)}   % Chinese title
\def\titleEn   {NCNU LaTeX Thesis Template(Chinese)} % English title
...
\edef\profs    {\profsZh}   % note, must use \edef -> expand now!
\def\author    {君の名は。}
\def\degreedateROC{中~華~民~國~一百零七~年~六~月}
\def\degreeyearEn {2018}
\def\degreeyearROC{107} % year of ROC
\def\degreemonth  {06} % MM , use 06 not 6
