...
\def\titleEn   {NTU LaTeX Thesis Template(Chinese)} % English title
...
% 臺大似乎只要附上審定書即可，請於 ``\letterVerif'' 填入檔名
% 如欲插入其他文書或調整順序請修改 
...
\def\wmContent{watermark.pdf} % 圖檔名或文字 | image filename or text.
\def\wmMethod{0} % {0}/{1} 0 == background pkg. , 1 == eso-pic pkg.
\def\wmScale{0.5}               % 縮放比
\def\wmOpacity{0.5}             % 透明度
\def\wmShiftFromCenterX{.5\paperwidth -43.25mm} % 圖片中心相對紙張中心垂直位移
\def\wmShiftFromCenterY{.5\paperheight-43.25mm} % 圖片中心相對紙張中心水平位移
\def\wmAngle{0}                 % 旋轉角度
% Note, below setting use NTU demo how to set.
% NTU Image size = 73mm^2, 
% require scale = 0.5 , image corner at 25mm from papers upper right.
% So shift = upper right(.5\paperwidth / .5\paperheight) 
%     - 73mm(img width) * 0.5(scale) / 2(half width) + 25mm
