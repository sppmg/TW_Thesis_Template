...
最後要編譯前記得對Zotero中的文獻庫（左方窗格的資料夾圖示）按滑鼠右鍵，選「匯出文獻庫」並選擇「BibLaTeX」格式（如果你打算採用BibTeX格式請記得自行修改樣版）匯出.bib文獻庫。如\cref{fig:zotero_exportBib}
...
各\LaTeX\ 目錄中的共用檔案及目錄(結尾含``/''者)如\cref{tab:template_structure} ，請盡量不要修改檔名或刪除檔案。
\begin{table}
    \centering
    \begin{tabularx}{\textwidth}{| l | X |}
        ...
        chapter\_*.tex & 各章節。（通常一章就一個檔。當然你可以再細分成更小的section子檔。\footnote{似乎在巢狀引入子擋上import套件比較好，請自行查閱說明手冊。}有些人會將chapter放在獨立目錄中，但以我的經驗，在搭配standalone等套件做子檔單獨編譯的情況下容易搞混相對路徑，所以我都放一起。）  \\ \hline
...
在 TeX Live 2016 上使用latexmk編譯後，輸出訊息結尾如下：（首行\$表示Linux下輸入指令）
\begin{lstlisting}[style=consoleStyle,language=bash]
$ latexmk -time -xelatex -interaction=errorstopmode main
....
....
....
Output written on main.pdf (69 pages).
SyncTeX written on main.synctex.gz.
Transcript written on main.log.
Latexmk: All targets (main.pdf) are up-to-date
'xelatex  -interaction=errorstopmode -recorder  "main.tex"': time = 18.7
'biber  "main"': time = 1.66999999999999
'xelatex  -interaction=errorstopmode -recorder  "main.tex"': time = 19.01
'xelatex  -interaction=errorstopmode -recorder  "main.tex"': time = 18.48
Accumulated processing time = 59.41
\end{lstlisting}
會發現xelatex 總共執行三次。實際上次數不固定，有時會見到 biber 執行兩次、xelatex執行超過3次的情況，所以若使用手動指定編譯次數方法，最好在編譯後檢視訊息，你可能會看到要求再次執行 biber 的訊息：
\begin{lstlisting}[style=consoleStyle,language=bash]
...
\subsection{字體大小}
字體大小預設10pt(1pt=1/72 inch)，設定在檔案最開頭如：
\begin{lstlisting}
documentclass[a4paper,10pt]{book}
\end{lstlisting}
不過10pt對於中文來說太小，而\LaTeX\ 預設文件類別又只提供有限的選項，且最高僅到12pt。
解決方法有兩種。一是在內文區段（\textbackslash{}begin\{document\}內）使用\textbackslash{}fontsize調整（由anyfontsize套件提供，本樣板中是由其他套件載入anyfontsize）。% 字高、行高
\begin{lstlisting}
...
\begin{table}[h]
    ...
    \end{tabular}
\end{table}
須要注意的是所有的大小都是相對於\textbackslash{}normalsize而訂，而這個\textbackslash{}normalsize就是一開始傳給class的數值。（本樣板因為歷史的因素使用了10pt，因此在使用\textbackslash{}fontsize調整後，會出現\textbackslash{}large比原文還小的情況，並且行高也會連帶受影響（
{\large 我是\textbackslash{}large啊！}
）。要確實處理這個問題只有使用其他class才能解決。）
...
\end{document}
\end{lstlisting}
除了首行之外，和一般tex檔完全一樣。首行class改用standalone，而選項中的class指定使用自訂樣式NCU\_thesis，crop=false設定不顯示邊框。

只要照上面的格式舊可以任意選擇編譯子檔還是主檔而不須另外處理。不過用\LaTeX\ 專用編輯器（如Texstudio）按快速鍵編譯者可能要注意編輯器可能會自動判斷主檔，導致即使你在子檔按下編譯鈕也還是選擇主檔進行編譯。
...
對於各檔共用套件，一般就是加入主檔。不過為了保持主檔的整潔，本樣板提供\textbackslash{}macro\_preamble.tex 作為共用導言區
\footnote{由.cls末端載入。}。
...
子文件：
\begin{lstlisting}
\documentclass[class=NCU_thesis, crop=false, draft]{standalone}
\end{lstlisting}
...
\begin{itemize}
 ...
 \item 字體大小不合適導致\textbackslash{}large比原文還小。須要更換基礎類別。
\end{itemize}
